...
const Application = () => {
  ...
  const [uiOptions, setUiOptions] = createSignal<UIOptions>({
  ...
  createEffect(() => {
    //@ts-ignore
    document.title =
      newsItems().length > 0 ? `News (${newsItems().length})` : "News";
  });
  ...
  const toggleBookmarks = () =>
    setUiOptions((opts) => ({ ...opts, showBookmarks: !opts.showBookmarks }));
                ...
                <div class="row">
                  <div class="col-12">
                    ...
                    <div class="row">
                      <div class="col-12">
                        <Show
                          when={uiOptions().showBookmarks}
                          children={
                            <BookmarksPanel
                              onDelete={application.bookmarks.del}
                              bookmarks={bookmarks()}
                            />
                          }
                        />
          ...
          <div class="col-sm-4 border-left border-right">
            <div class="row p-2">
              <div class="btn-group shadow-sm flex-fill" role="group">
                <HttpLiveStreamingRadio
                  title="RNZ"
                  playlistUrl="https://radionz.streamguys1.com/national/national/playlist.m3u8"
                />
                <HttpLiveStreamingRadio
                  title="George FM"
                  playlistUrl="https://livestream.mediaworks.nz/radio_origin/george_128kbps/playlist.m3u8"
                />
                <HttpLiveStreamingRadio
                  title="Hauraki"
                  playlistUrl="https://ais-nzme.streamguys1.com/nz_009/playlist.m3u8"
                />
                <HttpLiveStreamingRadio
                  title="Active"
                  playlistUrl="https://radio123-gecko.radioca.st/radioactivefm"
                />
              </div>
            </div>
            <div class="p-2" style="min-height:700px">
              <Weather
                forecasts={weather()}
                today={new Date()}
                link="https://www.metservice.com/towns-cities/locations/wellington/7-days"
              />
            </div>
            <div class="row p-2 justify-content-center">
              <Show
                when={seaTemp().length > 0}
                children={
                  <>
                    <div class="m-2 water-temp">
                      <ul class="list-group list-group-horizontal">
                        <For
                          each={seaTemp()}
                          children={(temp: WaterTemperature): Element => {
                            return (
                              <>
